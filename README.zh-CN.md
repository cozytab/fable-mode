...
**跨会话记忆**：超出单个项目的教训写进 `<配置目录>/fable-lessons.md`（一条一行，
最新在前）。Profile Injector 会在每个武装会话开局带出最前面几条——学过一次的教训
不再重学。这是官方文档明确记载的 Fable 5 优势之一，本 skill 用机械方式复现了它。

## 到底能逼近 Fable 5 到什么程度？

诚实回答：顶级产出 = 能力 × 纪律，本 skill 只能移植**纪律**这一半——但它把这一半
几乎移植完了，而且是机械强制而非口头承诺：设计门禁、逐卡验收、对抗验证、关卡带
证据、归因阶梯、跨会话教训、模型天花板，全部由 hook 和 lint 执行，不靠自觉。它给

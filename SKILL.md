...
**Promote lessons that transcend the project** to `<config-dir>/fable-lessons.md` (one bullet each, newest first; update rather than duplicate, delete wrong ones). The Profile Injector surfaces the top few in every armed session — this is the cross-session memory system: a lesson learned once stops being relearned.
